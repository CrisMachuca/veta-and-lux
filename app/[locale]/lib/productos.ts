// 1. TU TIPO ORIGINAL (Para mantener compatibilidad con lo que ya tienes hecho)
...
// 2. NUEVO TIPO PARA SANITY (Mapea exactamente tu nuevo panel de control)
export type ProductoSanity = {
  _id: string;
  nombre: string;
  slug: string;
  precio: number; // Ahora es un número puro para poder operar con él
  descripcion: string;
  descripcionLarga: string;
  imagen: any; // Sanity maneja las imágenes como objetos con metadatos
  imagenes?: any[]; // Array de imágenes secundarias
  materialBase: string;
  materialPantalla?: {
    tipo: string;
    color: string;
...
// 3. TU ARRAY DE PRODUCTOS LOCALES (Se queda aquí de forma temporal)
...
// 4. FUNCIÓN ORIGINAL DE BÚSQUEDA (Mantenida por compatibilidad)
