"use client"; // 🌟 OBLIGATORIO: Ahora maneja interacciones (abrir/cerrar)
  ...
  return (
    <nav className="sticky top-0 z-50 bg-stone-50/80 backdrop-blur-md border-b border-stone-200">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link
          href="/"
          onClick={() => setMenuAbierto(false)} // Cierra el menú si se pulsa el logo
        ...
        {/* CONTENEDOR DERECHO: Carrito + Botón Hamburguesa */}
        <div className="flex items-center gap-4 z-50">
          ...
          >
            {/* Animación de las líneas al transformarse en una "X" */}
            <span 
              className={`block h-0.5 w-6 bg-stone-800 transition-transform duration-300 ease-out rounded ${
                menuAbierto ? "rotate-45 translate-y-2" : ""
              }`} 
            />
            <span 
              className={`block h-0.5 w-6 bg-stone-800 transition-opacity duration-300 ease-out rounded ${
      ...
      {/* Fondo translúcido oscuro de fondo cuando el menú está abierto */}
      <div 
        className={`fixed inset-0 bg-stone-950/20 backdrop-blur-sm transition-opacity duration-300 md:hidden ${
      ...
      <div 
        className={`fixed top-0 right-0 h-screen w-64 bg-stone-50 border-l border-stone-200 p-8 pt-24 shadow-xl transition-transform duration-300 ease-in-out transform md:hidden ${
      ...
      >
        <div className="flex flex-col gap-6 text-sm uppercase tracking-widest text-stone-600 font-medium">
          <Link 
            href="/" 
            onClick={() => setMenuAbierto(false)}
            className="hover:text-stone-900 border-b border-stone-200/60 pb-3 transition-colors"
          >
            Inicio
          </Link>
          <Link 
